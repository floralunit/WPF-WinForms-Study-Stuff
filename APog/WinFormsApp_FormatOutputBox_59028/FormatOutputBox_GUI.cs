...
namespace WinFormsApp_FormatOutputBox
{
    ...
    public class FormatOutputBox_GUI // класс форматного вывода, содержащий ссылки на графические и текстовые объекты 
    {
        ...
        {
            None = 0,
            RShift, // = 1, т.к. в перечислимом типе значения по умолчанию увеличиваются на 1
            RShift_equal, // = 2
            XOR,
            XOR_equal
        };
            ...
            foreach (operations i in Enum.GetValues(typeof(operations))) //заполняем список операций
            {
                switch (i) // здесь необходимо заполнить список всех используемых операций в порядке их перечисления
                {
                    case operations.None:
                        _operations.Add(new
         FormatOutputBox_Handling.Operation_None());
                        break;
                    case operations.RShift:
                        _operations.Add(new
FormatOutputBox_Handling.Operation_RShift());
                        break;
                    case operations.RShift_equal:
                        _operations.Add(new
         FormatOutputBox_Handling.Operation_RShift_equal());
                        break;
                    case operations.XOR:
                        _operations.Add(new
         FormatOutputBox_Handling.Operation_XOR());
                        break;
                    case operations.XOR_equal:
                        _operations.Add(new
         FormatOutputBox_Handling.Operation_XOR_equal());
                        break;
                }
            }
        }
